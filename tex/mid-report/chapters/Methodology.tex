...
are many extensions to the basic Kalman filter (which is known as the Linear Kalman 
filter) allowing them to be used in variety of applications. Table \ref{table:ch:Comparison of different types of Kalman filters} provides a 
comparison between different types of Kalman filters. Note that the computational 
complexity of the filters increases from left to right.\\
\begin{table}[h]
\centering
...
\hline
Linear and Gaussian model&Non-linear and Gaussian model&Non-linear Gaussian model&Non-linear Gaussian model\\
\hline
...
dynamics of error using Jacobeans(Taylor series)&Calculate sigma points and use Unscented Transform to approximate the PDFs directly\\
\hline
...
\label{table:ch:A brief comparison of Kalman filters and Particle filters}
\end{table}
Three basic level filters were implemented and compared using the \gls{NCLT} data set to select the best filter for our application. Those filters were \gls{PF}, \gls{ESKF}, and \gls{UKF}. Then, the performance of the  best filter was improved by increasing the number of elements in the state vector, adding zero velocity update, and fussing relative measurements (Visual odometry , \gls{LiDAR} odometer) etc.The state vector used for basic filter implementations was \eqref{eq:basic state vector}
\begin{align}
\label{eq:basic state vector}
   \textbf{x} &= \left[\begin{matrix}{}\textbf{p}\\\textbf{v}\\\textbf{q}\end{matrix}\right]
\end{align}
where,\\
\textbf{p}=($p_x,p_y,p_z$) position relative to the inertial frame\\
\textbf{v}=($v_x,v_y,v_z$) velocity relative to the inertial frame\\
\textbf{q}=($q_\omega,q_x,q_y,q_z$) quaternion relative to the inertial frame\\
\\
The \gls{ESKF} estimates the error state directly and uses it as a correction to the nominal state.1000 particles were used in the \gls{PF}. Gaussian random noise was added to particles before applying the motion model to increase the diversity of particles. Systematic resample was used to resample particles when needed and squared error function was used as the weight function.1,1, and 2 were used as $\alpha$,k, and $\beta$ respectively in the unscented transform of \gls{UKF}.\\
The \gls{PF} had the worst orientation estimation while \gls{UKF} having the worst position estimation. According to the results, \gls{ESKF} performed way better than the other two filters, since it gave better estimations for both orientation and position. Also, the computational complexity of \gls{ESKF} was low with compared to other filters. Hence, \gls{ESKF} was selected as the best filter to tackle our problem.
