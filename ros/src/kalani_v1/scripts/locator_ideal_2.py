...
def visualodom_callback(data):
    ...
    v_ci = dp_ci / (t0-t1) # velocity in camera init frame
    v_c0 = np.matmul(ciRc0.T, v_ci) # velocity in old camera frame
    v_v = np.matmul(vRc, v_c0) # velocity in vehicle frame
    v_v[0] = 0 # zero velocity update
    v_v[2] = 0
    log.log("velocity in vehicle frame:{}, time diff:{}".format(v_v, t1-t0))
    ...
    log.log("orientation difference:{}".format(dtheta))
        
    if tft.vector_norm(v_v) < 1e-2:
        stationary = True
    else:
        stationary = False
