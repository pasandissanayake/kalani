...
class RB_Particle_Filter_V1():
    def __init__(self):
        ...
        self._kf_state = np.zeros([self.NO_OF_PARTICLES, 6])
        self._kf_covariance = np.zeros([self.NO_OF_PARTICLES, 6, 6])
        self._filter_time = -1
        self._mean_state = np.zeros(10)
        self._mean_covariance = np.zeros((6, 6))
    ...
    def predict(self, am, var_am, wm, time):
        ...
        self._kf_state[:, 0:3] = self._kf_state[:, 0:3] + self._kf_state[:, 3:6] * dt + 0.5 * (matrix_vector_multiply(R, am) + self._g) * dt ** 2
        self._kf_state[:, 3:6] = self._kf_state[:, 3:6] + (matrix_vector_multiply(R, am) + self._g) * dt

        F = np.full((self.NO_OF_PARTICLES,6,6),np.eye(6))
        F[:, 0:3, 3:6] = np.eye(3) * dt

        B = np.zeros((self.NO_OF_PARTICLES, 6, 3))
    ...
    def correct(self, fix, var_fix, time):
        ...
        self._kf_covariance = matrix_matrix_multiply(np.eye(6) - matrix_matrix_multiply(K, H), self._kf_covariance)
    ...
    def get_state_as_numpy(self):
        return np.concatenate([self._mean_state[0:6], self._mean_state[6:10], [self._mean_filter_time, float(self._filter_initialized)]])
...
for i in range(1000):
    ...
    if t_imu < t_gps:
        ...
        if pf.get_state_as_numpy()[-1] < 1:
            ...
            pf.initialize_state(q=q, cov_q=np.array([0.01, 0.01, 1]), time=t_imu)
            skip_pause = True
        else:
            print 'prediction'
            pf.predict(fm, np.ones(3)*0.001, wm, t_imu)

            the_state.append(pf.get_state_as_numpy()[0:3])
            state = np.array(the_state)
        ...
        if pf.get_state_as_numpy()[-1] < 1:
            print 'gps init'
            pf.initialize_state(p=fix, cov_p=np.ones(3)*1, v=np.zeros(3), cov_v=np.ones(3)*10, ab=np.zeros(3), cov_ab=np.ones(3)*0.0000001, g=np.array([0, 0, -9.8]), time=t_gps, initialized=True)

            q = pf.get_state_as_numpy()[6:10]
            yaw = tft.euler_from_quaternion(q)[2]
            print yaw
            plt.arrow(fix[0], fix[1], 0.01 * np.cos(yaw), 0.01 * np.sin(yaw), head_width=0.0005, head_length=0.001, fc='k', ec='k')
            plt.draw()
            ...
            pf.correct(fix[0:2], np.ones(2)*10, t_gps)

            particles.append(pf.get_corrections())
            particles.append(pf.get_positions_of_all_particles_as_numpy())
            particles = np.array(particles)
            for j in range(pf.NO_OF_PARTICLES):
                plt.plot([particles[0, j, 0], particles[1, j, 0]], [particles[0, j, 1], particles[1, j, 1]], linestyle='dashed', marker='v', color='red')
