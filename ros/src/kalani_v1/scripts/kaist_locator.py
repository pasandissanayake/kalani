...
def gnss_callback(data):
    ...
    if kf.is_initialized():
        Hx = np.zeros([3, 16])
        Hx[:, 0:3] = np.eye(3)
        V = np.diag([covariance[0],covariance[4],covariance[8]])
        ...
        def hx_func(state):
            ...
            Hx = np.array([
                [0, 0, 0, 1 - 2 * (qy ** 2 + qz ** 2), 2 * (qx * qy + qz * qw), 2 * (qx * qz - qy * qw),
                 ...
                 2 * (qw * vy + qx * vz - 2 * qz * vz), 0, 0, 0, 0, 0, 0],
...
def laser_callback(data):
    ...
    try:
        ...
        time = data.header.stamp - rospy.Duration(1)
        odom2am_transform = tf2_buffer.lookup_transform('odom', 'aft_mapped', time)
        lv2bl_transform = tf2_buffer.lookup_transform('left_velodyne', 'base_link', time)
        q1 = [odom2am_transform.transform.rotation.x, odom2am_transform.transform.rotation.y,
              odom2am_transform.transform.rotation.z, odom2am_transform.transform.rotation.w]
        q2 = [lv2bl_transform.transform.rotation.x, lv2bl_transform.transform.rotation.y,
              lv2bl_transform.transform.rotation.z, lv2bl_transform.transform.rotation.w]
        q = tft.quaternion_multiply(q1, q2)
        transform = TransformStamped()
        transform.header.stamp = data.header.stamp
        transform.header.frame_id = 'odom'
        transform.child_frame_id = 'laser_data'
        transform.transform.translation.x, transform.transform.translation.y, transform.transform.translation.z = (5, 10, 0)
...
if __name__ == '__main__':
    ...
    world2map_tf = TransformStamped()
    world2map_tf.header.stamp = rospy.Time.now()
    world2map_tf.header.frame_id = 'world'
    world2map_tf.child_frame_id = 'map'
    world2map_tf.transform.translation.x = 0
    world2map_tf.transform.translation.y = 0
    world2map_tf.transform.translation.z = 0
    world2map_tf.transform.rotation.w = 1
    world2map_tf.transform.rotation.x = 0
    world2map_tf.transform.rotation.y = 0
    world2map_tf.transform.rotation.z = 0
    # tf2_static_broadcaster1.sendTransform(world2map_tf)

    # map to odom static transformation
    map2odom_tf = TransformStamped()
    map2odom_tf.header.stamp = rospy.Time.now()
    map2odom_tf.header.frame_id = 'map'
    map2odom_tf.child_frame_id = 'odom'
    map2odom_tf.transform.translation.x = 0
    map2odom_tf.transform.translation.y = 0
    map2odom_tf.transform.translation.z = 0
    map2odom_tf.transform.rotation.w = 1
    map2odom_tf.transform.rotation.x = 0
    map2odom_tf.transform.rotation.y = 0
    map2odom_tf.transform.rotation.z = 0
    # tf2_static_broadcaster2.sendTransform(map2odom_tf)

    # base_link to left_velodyne static transformation
    bl2lv_tf = TransformStamped()
    bl2lv_tf.header.stamp = rospy.Time.now()
    bl2lv_tf.header.frame_id = 'base_link'
    bl2lv_tf.child_frame_id = 'left_velodyne'
    bl2lv_tf.transform.translation.x = -0.397052
    bl2lv_tf.transform.translation.y = -0.440699
    bl2lv_tf.transform.translation.z = 1.90953
    q = tft.quaternion_from_matrix(np.array([[-0.486485, 0.711672, -0.506809, -0.397052],
               [-0.514066, -0.702201, -0.492595, -0.440699],
               [-0.706447, 0.0208933, 0.707457, 1.90953],
               [0., 0., 0., 1.]]))
    bl2lv_tf.transform.rotation.w = q[3]
    bl2lv_tf.transform.rotation.x = q[0]
    bl2lv_tf.transform.rotation.y = q[1]
    bl2lv_tf.transform.rotation.z = q[2]
    # tf2_static_broadcaster3.sendTransform(bl2lv_tf)

    # base_link_init to camera_init static transformation
    bli2ci_tf = TransformStamped()
    bli2ci_tf.header.stamp = rospy.Time.now()
    bli2ci_tf.header.frame_id = 'base_link_init'
    bli2ci_tf.child_frame_id = 'camera_init'
    bli2ci_tf.transform.translation.x = -0.397052
    bli2ci_tf.transform.translation.y = -0.440699
    bli2ci_tf.transform.translation.z = 1.90953
    q = tft.quaternion_from_matrix(np.array([[-0.486485, 0.711672, -0.506809, -0.397052],
                                             [-0.514066, -0.702201, -0.492595, -0.440699],
                                             [-0.706447, 0.0208933, 0.707457, 1.90953],
                                             [0., 0., 0., 1.]]))
    bli2ci_tf.transform.rotation.w = q[3]
    bli2ci_tf.transform.rotation.x = q[0]
    bli2ci_tf.transform.rotation.y = q[1]
    bli2ci_tf.transform.rotation.z = q[2]
    tf2_static_broadcaster.sendTransform([bli2ci_tf, world2map_tf, map2odom_tf, bl2lv_tf])
