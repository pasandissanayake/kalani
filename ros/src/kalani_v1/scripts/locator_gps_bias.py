...
def mag_callback(data):
    ...
    if kf.is_initialized:
        ...
        kf.initialize([
            ...
            ['ab', init_imu_linear_acceleration_bias, np.diag(var_imu_linear_acceleration_bias), t],
            ['wb', init_imu_angular_velocity_bias, np.diag(var_imu_angular_velocity_bias), t],
            ['gb', init_gnss_bias, np.diag(var_gnss_bias), t]
        ])
...
def visualodom_callback(data):
    ...
    try:
        ...
        return
    q_cv = np.array([trans.transform.rotation.x, trans.transform.rotation.y, trans.transform.rotation.z, trans.transform.rotation.w])
    R_cv = tft.quaternion_matrix(q_cv)[0:3,0:3]
    ...
    # kf.correct_absolute(meas_fun, v_v, np.eye(3)*1e-4, t, measurement_name='visualodom')
...
if __name__ == '__main__':
    ...
    log = Log('g' + config['locator_node_name'])
