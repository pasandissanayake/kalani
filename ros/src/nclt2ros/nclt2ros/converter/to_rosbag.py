    ...
    def process(self):
        ...
        while not rospy.is_shutdown():
            ...
            if next_packet == "done":
                ...
                # print("write gps")
                navsat, track, speed, timestamp = self.ros_sensor_msg.gps_to_navsat(gps_list=gps_list, i=i_gps)
                ...
                # print("write gps_rtk")
                navsat, track, speed, timestamp = self.ros_sensor_msg.gps_rtk_to_navsat(gps_rtk_list=gps_rtk_list, i=i_gps_rtk)
                ...
                # print("write ms25")
                imu, mag, timestamp = self.ros_sensor_msg.ms25_to_imu(imu_list=ms25_list, i=i_ms25)
                ...
                # print("write odom")
                odom, timestamp = self.ros_sensor_msg.wheel_odom_to_odometry(wheels_list=wheels_list, i=i_odom)
                self.bag.write(self.odom_topic, odom, t=timestamp)
                i_odom += 1

            elif next_packet == "gt":
                # print("write gt")
                gt, timestamp = self.ros_sensor_msg.gt_to_odometry(gt_list=gt_list, gt_cov_list=gt_cov_list, i=i_gt)
