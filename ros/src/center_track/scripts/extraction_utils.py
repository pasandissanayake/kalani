...
class Box3D(object):
    ...
    def __init__(self, data ):
        ...
        self.xmin = data[0]  # left
        self.ymin = data[1]  # top
        self.xmax = data[2]  # right
        ...
        self.ry = data[10]  # yaw angle (around Y-axis in camera coordinates) [-pi..pi]
...
def project_to_image3d(points, proj_mat):
    """
    Apply the perspective projection
    Args:
        pts_3d:     3D points in camera coordinate [3, npoints]
...
def map_box_to_image(box, proj_mat):
    ...
    points_2d , points_depth = project_to_image(points_3d, proj_mat)
...
def draw_projected_box3d(image, qs, color=(255, 255, 255), thickness=1):
    ...
    for k in range(0, 4):
        ...
        #cv2.putText(image, str(i), ( qs[i, 0], qs[i, 1] ) , cv2.FONT_HERSHEY_SIMPLEX , 0.5 , (255, 0, 0) , 1 , cv2.LINE_AA)
        #cv2.putText(image, str(j) , ( qs[j, 0], qs[j, 1] ) , cv2.FONT_HERSHEY_SIMPLEX , 0.5 , (255, 0, 0) , 1 , cv2.LINE_AA)
