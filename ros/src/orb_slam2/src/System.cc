...
void System::SaveNumMatches(const string &filename)
{
	cout << endl << "Saving Number of matches to " << filename << " ..." << endl;
	
	vector<int> numkey = mpTracker->NumMatcheslist;
	
	vector<int> numkeymap = mpTracker->NumMatchesVisualist;

    ofstream f;
    f.open(filename.c_str());
    f << fixed;
    for(size_t i=0; i<numkey.size(); i++)
    {
        f << setprecision(9) << numkey.at(i) << " "  <<numkeymap.at(i)<< endl;

    }

    f.close();	
    cout << endl << "Matches saved!" << endl;
	
}

void System::SaveKeyFramesTest(const string &filename)
{
	cout << endl << "Saving Keyframes test to " << filename << " ..." << endl;
	
	vector<KeyFrame*> numkey = mpTracker->Keyframesout;
...
std::vector<double> System::PublishKeyFrame()
{
	
	int newKeyFrameID = mpTracker-> KeyFrameID;
	
	KeyFrame* numkey = mpTracker->KeyFramePublish;
    ...
    if (newKeyFrameID>lastKeyFrameID)
    {
	   ...
	   KeyFramet.insert(KeyFramet.begin()+4,q[2]);
	   KeyFramet.insert(KeyFramet.begin()+5,q[3]);
	   ...
	   return KeyFramet;
		...
		//cout << endl << "Not Publishing Keyframes " << endl;
		vector<double> KeyFramet = {0.0};
        lastKeyFrameID = newKeyFrameID;
		return KeyFramet;
